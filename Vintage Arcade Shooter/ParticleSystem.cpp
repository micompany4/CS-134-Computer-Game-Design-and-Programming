...
void ImpulseRadialForce::updateForce(Particle * particle) {
	...
	//
	if (height < 5)		//checks if the height is less than a certain value, if so then a controlled, directional explosion will occur
	{
		updateForce2(particle);
		return;
	}
...
//used to create the ring-like explosion
void ImpulseRadialForce::updateForce2(Particle * p)
{
	ofVec3f dir = ofVec3f(ofRandom(-1, 1), height, ofRandom(-1, 1));	//height is used to "clamp" the Y value of the directional vector
	p->forces += dir.getNormalized() * magnitude;
}